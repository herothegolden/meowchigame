// src/pages/ShopPage.jsx - Complete with audio integration
...
const ShopItemCard = ({ item, userPoints, onPurchase, isOwned, ownedQuantity = 0, isPurchasing, justPurchased }) => {
  ...
  const handlePurchaseClick = () => {
    playButtonClick(); // Audio feedback for purchase attempt
    onPurchase(item.id);
  };
...
const ShopPage = () => {
  ...
  // ENHANCED PURCHASE FUNCTION WITH AUDIO FEEDBACK
  const handlePurchase = async (itemId) => {
    ...
    try {
      if (isConnected && tg && tg.initData && BACKEND_URL) {
        ...
        // AUDIO: Success sounds for successful purchase!
        ...
        // Demo mode - instant updates with audio
        if (userPoints >= item.price) {
          // AUDIO: Success sound for demo purchase
          playItemActivate();
          setTimeout(() => playScoreUpdate(), 100);
