...
export default function GameView({ onGameOver }) {
  ...
  // 🆕 PHASE 3: Special Cat Detection Functions
  ...
  const detect5InLine = (grid, matches) => {
    ...
    // Check for exactly 5 in a line (horizontal or vertical) for Catnip Bomb
    for (let r = 0; r < ROWS; r++) {
      let streak = 1;
      let currentCat = null;
      for (let c = 1; c < COLS; c++) {
  ...
  const detectLTShape = (grid, matches) => {
    ...
    // Simplified L/T detection for Phase 3
    ...
    if (matches.length >= 5) {
      ...
      if (hasHorizontal && hasVertical && matches.length >= 5) {
        const originalCat = grid[centerR] && grid[centerR][centerC] ? grid[centerR][centerC] : matches[0] && grid[matches[0][0]] ? grid[matches[0][0]][matches[0][1]] : CAT_SET[0];
        specials.push({
          r: centerR,
          c: centerC,
  ...
  // 🆕 PHASE 3: Special Activation Functions
  ...
  // 🆕 PHASE 3: Handle special cat click
  const handleSpecialClick = useCallback((r, c, specialType, originalCat) => {
    ...
    switch(specialType) {
      case SPECIAL_TYPES.WHISKER_STREAK_H:
        result = activateWhiskerStreak(r, c, 'horizontal', originalCat);
        break;
      case SPECIAL_TYPES.WHISKER_STREAK_V:
        result = activateWhiskerStreak(r, c, 'vertical', originalCat);
        break;
      case SPECIAL_TYPES.BOX_CAT:
        result = activateBoxCat(r, c, originalCat);
        break;
      case SPECIAL_TYPES.CATNIP_BOMB:
        result = activateCatnipBomb(r, c, originalCat);
        break;
      default:
        return;
    }
    ...
    if (result) {
      ...
      // Clear selected tile and continue with cascades
      setSelectedTile(null);
      
      // Trigger cascade after special activation
      setTimeout(() => {
        cascadeAndFillGrid(result.grid, result.specialGrid);
      }, 500);
    }
  }, [gameActive, isPaused]);
  ...
  // Enhanced cascade function with special detection
  ...
  function processMatches(currentGrid, currentSpecialGrid, matches, cascadeLevel = 1) {
    // 🆕 PHASE 3: Detect specials BEFORE removing matches
    ...
    // Place special cats first (they replace normal matches)
    ...
    // Remove matched tiles (except where specials were placed)
    matches.forEach(([r, c]) => {
      if (!sg[r][c]) { // Only remove if no special was placed here
        g[r][c] = null;
        sg[r][c] = null;
      }
    });
  ...
  // Game logic: tile selection and swapping
  ...
  return (
    <div className="section board-wrap">
      ...
      >
        ...
        {/* 🆕 PHASE 3: Special Effects Overlay */}
        {specialEffects.map(effect => {
          if (effect.type === 'line-clear') {
            return (
              <div
                key={effect.id}
                className="special-activation-effect line-clear-effect"
                style={{
            ...
            return (
              <div
                key={effect.id}
                className="special-activation-effect area-blast-effect"
                style={{
                  left: (effect.c - 1) * cellSize,
                  top: (effect.r - 1) * cellSize,
                  width: cellSize * 3,
                  height: cellSize * 3,
                }}
              />
            ...
            return (
              <div
                key={effect.id}
                className="special-activation-effect color-bomb-effect"
                style={{
                  left: effect.c * cellSize - cellSize,
                  top: effect.r * cellSize - cellSize,
                  width: cellSize * 3,
                  height: cellSize * 3,
                }}
              />
