...
// Diagnostic Component for troubleshooting leaderboard issues
    ...
    <div style={{
      ...
      {diagnostics.databaseStatus && (
        <div style={{
          ...
          {diagnostics.databaseStatus.leaderboardTest && (
            <div style={{ marginTop: '10px' }}>
              <h5>🏆 Direct Leaderboard Query Test:</h5>
              <div>Found {diagnostics.databaseStatus.leaderboardTest.length} players with games</div>
              {diagnostics.databaseStatus.leaderboardTest.slice(0, 3).map((player, i) => (
                <div key={i} style={{ fontSize: '12px', padding: '2px 0' }}>
                  {i + 1}. {player.display_name} - {player.total_score} points
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{
          ...
          {diagnostics.leaderboardTest.leaderboard?.length === 0 && (
            <div style={{
              ...
              ⚠️ LEADERBOARD EMPTY - API returning no players!
            </div>
          )}

          {diagnostics.leaderboardTest.leaderboard?.length > 0 && (
            <div style={{ marginTop: '10px' }}>
              <h5>Top Players from API:</h5>
              {diagnostics.leaderboardTest.leaderboard.slice(0, 3).map((player, i) => (
                <div key={i} style={{ fontSize: '12px', padding: '2px 0' }}>
                  {player.rank}. {player.display_name} - {player.total_score} points
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{
        ...
        <ol style={{ margin: '10px 0', paddingLeft: '20px' }}>
          ...
          <li>If leaderboard API shows 0 players but database has games → Check server logs</li>
          <li>If game submission fails → Check browser console for errors</li>
          <li>After adding games, go to Rankings tab to see leaderboard</li>
        </ol>
    ...
    <>
      ...
      <div className="shell" style={{ visibility: showSplash ? "hidden" : "visible" }}>
        ...
        <main className="content">
          {/* DIAGNOSTIC PANEL - Can be toggled in Settings */}
          {showDiagnostics && (
